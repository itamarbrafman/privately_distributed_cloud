#ifndef ILRD_RD141_WAITABLE_QUEUE_HPP
#define ILRD_RD141_WAITABLE_QUEUE_HPP
...
bool WaitableQueue<T, CONTAINER>::Pop(T &outparam, std::chrono::nanoseconds timeout)
{
    ...
    while (m_container_queue.empty())
    {
        if(m_cond_var.wait_for(lock, timeout) == std::cv_status::timeout)
        {
            return false;
        }
    }
...
#endif // ILRD_RD141_WAITABLE_QUEUE_HPP